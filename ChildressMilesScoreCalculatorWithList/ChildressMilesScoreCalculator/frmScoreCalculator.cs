...
namespace ChildressMilesScoreCalculator
{
    public partial class frmScoreCalculator : Form
    {
        ...
        int scoreTotal;
      //  int scoreCount; deleted for this project
        int averageScore;
       ...
       List<int> intScoresList = new List<int>();
        ...
        private void BTNAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsItValidData())
                {
                   ...
                   intScoresList.Add(int.Parse(TXTScore.Text));
                    // scoreCount++;
                    int intScoreCount = intScoresList.Count;
                    averageScore = scoreTotal / intScoreCount;
        ...
        #endregion
        //validation
        private string IsItPresent(string value, string name)
        {
            string msg = "";
            if (value == "")
            {
        ...
        private string IsItInt(string value, string name)
        {
            string msg = "";
            if (!int.TryParse(value, out _))
            {
        ...
        private bool IsItValidData()
        {
            bool success = true;
            string errorMessage = "";

            //validate operand1 and operand2 for IsDecimal and IsWithinRange
            errorMessage += IsItInt(TXTScore.Text, TXTScore.Tag.ToString());
            errorMessage += IsItPresent(TXTScore.Text, TXTScore.Tag.ToString());
